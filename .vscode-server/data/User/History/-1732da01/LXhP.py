...
def add_training_record():
    ...
    training = input("훈련 이름을 입력하세요: ")
    success = input("훈련 성공 여부를 입력하세요 (y/n): ").lower() == 'y'
    date = input("훈련 날짜를 입력하세요 (예: 2025-01-01): ")

    cursor.execute("""
    INSERT INTO training_records (pet_id, training, success, date)
    VALUES (?, ?, ?, ?)
    """, (pet_id, training, success, date))
    conn.commit()
    conn.close()
