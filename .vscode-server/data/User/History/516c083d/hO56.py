...
from user_management import set_user_profile, login  # 사용자 관리 관련 함수 (프로필 생성 및 로그인)를 가져옵니다.
from pet_management import add_pet, update_pet, delete_pet, view_my_pets  # 반려동물 관리 관련 함수 (등록, 수정, 삭제, 조회)를 가져옵니다.
...
def execute_family_choice(choice, logged_in_user):  # 나의 가족 보기 메뉴에서 선택된 옵션을 실행하는 함수의 정의
    match choice:  # 선택된 옵션에 따라 실행할 작업을 결정
        ...
        case "3":
            delete_pet()
        case "4":
            view_my_pets(logged_in_user["username"])
        case "5":
...
def execute_training_choice(choice, logged_in_user):  # 훈련 기록 메뉴에서 선택된 옵션을 실행하는 함수의 정의
    match choice:
        case "1":
            add_training_record(logged_in_user["username"])
        case "2":
            view_training_records(logged_in_user["username"])
        case "3":
            return False
        case _:
            print("잘못된 입력입니다. 다시 시도해주세요.")
...
def main():  # 프로그램의 진입점으로, main 루트 실행하는 함수
    ...
    while True:  # 사용자와의 상호작용을 계속해서 처리하기 위한 무한 루프
        if not logged_in_user:  # 로그인 상태가 아닌 경우 메인 메뉴를 표시함
            ...
            match choice:  # 선택된 옵션에 따라 실행할 작업 결정
                case "1":
