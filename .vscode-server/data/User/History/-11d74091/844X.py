...
def delete_pet():
    ...
    while True:
        confirm = input(f"정말로 반려동물(ID: {pet_id})을 삭제하시겠습니까? (y/n): ").lower()
        if confirm == 'y':
...
def update_pet_notes():
    ...
    pet_id = int(input("특성을 추가/수정할 반려동물 ID를 입력하세요: "))
    notes = input("반려동물 특성을 입력하세요: ")

    cursor.execute("UPDATE pets SET notes = ? WHERE id = ?", (notes, pet_id))
    conn.commit()
    conn.close()
    print("반려동물 특성이 성공적으로 추가/수정되었습니다!")
